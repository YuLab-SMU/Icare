...
train_lasso_base <- function(train_data,
                             ...
                             save_plot = TRUE,
                             save_dir = here("PrognosiX", "lasso_model"),
                             plot_width = 5,
                             plot_height = 5,
                             base_size = 14,
                             seed = 123) {
  ...
  y_train <- Surv(train_data[[time_col]], train_data[[status_col]])

  cv_fit <- cv.glmnet(x_train, y_train, family = "cox", alpha = 1, nfolds = nfolds)
  best_lambda_value <- cv_fit$lambda.min

  fit_best_cv_lasso <- glmnet(x_train, y_train, family = "cox", alpha = 1, lambda = best_lambda_value)
  ...
  p <- ggplot(cv_data, aes(x = lambda, y = mean_error)) +
...
evaluate_lasso_roc <- function(
  ...
  if (!is.null(train_data)) {
    ...
    lasso_roc_train <- roc(train_data[[status_col]], as.numeric(lasso_risk_score_train))
  ...
  if (!is.null(test_data)) {
    ...
    lasso_roc_test <- roc(test_data[[status_col]], as.numeric(lasso_risk_score_test))
    ...
    Test <- data.frame(
      C_index = concordance.index(x = lasso_risk_score_test,
                                  surv.time = test_data[[time_col]],
                                  surv.event = test_data[[status_col]],
                                  method = "noether")$c.index,
      ROC_AUC = auc(lasso_roc_test)
    )
  ...
  if (!is.null(validation_data)) {
    ...
    lasso_roc_validation <- roc(validation_data[[status_col]], as.numeric(lasso_risk_score_validation))
    ...
    Validation <- data.frame(
      C_index = concordance.index(x = lasso_risk_score_validation,
                                  surv.time = validation_data[[time_col]],
                                  surv.event = validation_data[[status_col]],
                                  method = "noether")$c.index,
      ROC_AUC = auc(lasso_roc_validation)
    )
...
evaluate_lasso_km <- function(
  ...
  fit_km_lasso <- survfit(Surv(data[[time_col]], data[[status_col]]) ~ risk_group, data = data)
  cat("Kaplan-Meier fit for lasso risk groups completed.\n")
  
  km_pval_lasso <- pchisq(survdiff(Surv(data[[time_col]], as.numeric(data[[status_col]])) ~ risk_group,
                                   data = data)$chisq, 1, lower.tail = FALSE)
  
  km_hr_lasso <- (survdiff(Surv(data[[time_col]], as.numeric(data[[status_col]])) ~ risk_group,
                           data = data)$obs[2] / survdiff(Surv(data[[time_col]], as.numeric(data[[status_col]])) ~ risk_group,
                                                          data = data)$exp[2]) /
    (survdiff(Surv(data[[time_col]], as.numeric(data[[status_col]])) ~ risk_group,
              data = data)$obs[1] / survdiff(Surv(data[[time_col]], as.numeric(data[[status_col]])) ~ risk_group,
                                             data = data)$exp[1])
  
  km_upper95_lasso <- exp(log(km_hr_lasso) + qnorm(0.975) * sqrt(1 / survdiff(Surv(data[[time_col]], as.numeric(data[[status_col]])) ~ risk_group,
                                                                              data = data)$exp[2] + 1 / survdiff(Surv(data[[time_col]], as.numeric(data[[status_col]])) ~ risk_group,
                                                                                                                 data = data)$exp[1]))
  km_lower95_lasso <- exp(log(km_hr_lasso) - qnorm(0.975) * sqrt(1 / survdiff(Surv(data[[time_col]], as.numeric(data[[status_col]])) ~ risk_group,
                                                                              data = data)$exp[2] + 1 / survdiff(Surv(data[[time_col]], as.numeric(data[[status_col]])) ~ risk_group,
                                                                                                                 data = data)$exp[1]))
  ...
  surv_plot <- km_plot_lasso$plot + theme_prism(base_size = base_size) +
    theme(
  ...
  risk_table <- km_plot_lasso$table + theme_prism(base_size = base_size) +
    theme(
      plot.title = element_text(hjust = 0.5, face = "bold", size = 14),
      axis.text.x = element_text(angle = 45, hjust = 1, size = 10),
      axis.text.y = element_text(size = 10),
...
lasso_compute_hr_and_ci <- function(object,
  ...
  if (inherits(object, 'PrognosiX')) {
    ...
    time_col <- slot(object, "time_col")
    fit_best_cv_lasso <- slot(object, "survival.model")[["lasso_model"]][["model"]]
    lambda<-slot(object, "survival.model")[["lasso_model"]][["lambda"]]
...
forest_plot_lasso_model <- function(object,
  ...
  if (inherits(object, 'PrognosiX')) {
    hr_results <- object@survival.model[["lasso_model"]][["hr_results"]]

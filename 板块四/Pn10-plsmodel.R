...
train_pls_base <- function(train_data,
  ...
  fit_best_cv_pls <- plsRcox(x_train, y_train, lambda = best_lambda)
...
evaluate_pls_roc <- function(
    ...
    status_col = "status",  
    save_plot = TRUE,   
    save_dir = here("PrognosiX", "pls_model"),  
    plot_width = 7,    
    plot_height = 7,   
    base_size = 14     
) {
  ...
  if (!is.null(validation_data)) {
    ...
    Validation <- data.frame(
      C_index = survcomp::concordance.index(x = pls_risk_score_validation,
                                            ...
                                            method = "noether")$c.index,
      ROC_AUC = auc(pls_roc_validation)
    )
...
evaluate_pls_km <- function(fit_best_cv_pls,
  ...
  km_fit_pls <- survfit(Surv(data[[time_col]], as.numeric(data[[status_col]])) ~ risk_group, data = data)
  
  survdiff_result_pls <- survdiff(Surv(data[[time_col]], as.numeric(data[[status_col]])) ~ risk_group, data = data)
  ...
  risk_table <- km_plot_pls$table + theme_prism(base_size = base_size) +
    theme(
      plot.title = element_text(hjust = 0.5, face = "bold", size = 14),
      axis.text.x = element_text(angle = 45, hjust = 1, size = 10),
      axis.text.y = element_text(size = 10),
...
forest_plot_pls_model <- function(object,
  ...
  if (inherits(object, 'PrognosiX')) {
    hr_results <- object@survival.model[["pls_model"]][["hr_results"]]

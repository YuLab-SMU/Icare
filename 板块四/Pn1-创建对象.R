...
CreatePrognosiXObject <- function(
  ...
  if (nrow(info.data) == 0) {
    if (!is.null(time_col) && !is.null(status_col)) {
      ...
      colnames(info.data)[colnames(data) == time_col]<- "time"
      colnames(info.data)[colnames(data) == status_col]<- "status"
...
ExtractCleanData<-function(object) {
  data <- tryCatch(slot(object, "clean.data"),
...
ExtractInfoData<-function(object) {
  data <- tryCatch(slot(object, "info.data"), error = function(e) NULL)
  return(data)
}

...
CreateSubtypingObject <- function(
  ...
  if (!is.null(object)) {
    if (!inherits(object, "Stat") && !inherits(object, "Subtyping") && 
        !inherits(object, "PrognosiX") && !inherits(object, "Model_data")) {
      stop("The 'object' parameter must be an instance of class 'Stat', 'Subtyping', 'Model_data'or 'PrognosiX'.")
    }
    ...
    else if (inherits(object, "Model_data")) {
